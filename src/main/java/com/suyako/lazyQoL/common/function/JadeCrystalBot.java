...
import com.suyako.lazyQoL.utils.Vec3Comparable;
    ...
    public void process(ClientChatReceivedEvent event) {
        ...
        if (currentState != originalState || originalCount != possibleBlocks.size()) {
            switch (currentState) {
                case FOUND_KNOWN:
                case FOUND:
                    Utils.print("[JadeCrystalBot] find target");
                    break;
                case INVALID:
                case FAILED:
                    Utils.print("[JadeCrystalBot] cannot find target");
                    needMove = true;
                    resetSolution(false);
                    break;
                case MULTIPLE_KNOWN:
                    // falls through
                case MULTIPLE:
                    Utils.print("[JadeCrystalBot] need to move to another position to find the target, possible block: " + possibleBlocks.size());
                    needMove = true;
                    break;
                default:
    ...
    private static boolean locateMinesCenterIfNeeded() {
        ...
        if (keeperEntities.size() == 0) {
            if (!visitKeeperMessagePrinted) {
                Utils.print("hand Metal Detector and close Keeper to enable fast search");
        ...
        Utils.print("Keeper exist, fast search enabled");
        return true;
    }
