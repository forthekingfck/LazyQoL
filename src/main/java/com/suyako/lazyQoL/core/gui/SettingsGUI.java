...
public class SettingsGUI extends GuiScreen {
    ...
    public void initGui() {
        ...
        BackButton = new GuiClickGUIButton(0,10,this.height - 25,widthBound - 10,18,"Save and Back",new ResourceLocation(LazyQoL.MODID,"clickgui/back.png"));;
    ...
    public void drawScreen(int x, int y, float delta)
    {
        ...
        FontManager.QuicksandFont35.drawString("LazyQoL",2,2,new Color(255,255,255).getRGB());
        drawRect(0,20,widthBound - pos,scaledResolution.getScaledHeight(),new Color(255,255,255,128).getRGB());
        String title = guiName + " config";
    ...
    private void handleBoolean(SettingBoolean option){
        ...
        utils.print(guiName + "‘s " + option.name + "was setting to " + ((Boolean)option.getValue() ? "enabled" : "disabled"));
    }
    private void handleTypeSelector(SettingTypeSelector option){
        option.switchStatus();
        Utils utils = new Utils();
        utils.print(guiName + "'s " + option.name + "was setting to " + (option.getKey()));
    }
