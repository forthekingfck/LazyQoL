...
import com.suyako.lazyQoL.utils.FutureTaskId;
...
public class Platform implements ViaPlatform<UUID> {
    ...
    public FutureTaskId runAsync(Runnable runnable) {
        return new FutureTaskId(CompletableFuture
                .runAsync(runnable, LazyQoL.getAsyncExecutor())
    ...
    public FutureTaskId runSync(Runnable runnable) {
        return new FutureTaskId(LazyQoL.getEventLoop().submit(runnable).addListener(errorLogger()));
    }

    @Override
    public PlatformTask runSync(Runnable runnable, long ticks) {
        return new FutureTaskId(LazyQoL.getEventLoop().schedule(() -> runSync(runnable), ticks *
                50, TimeUnit.MILLISECONDS).addListener(errorLogger()));
    }

    @Override
    public PlatformTask runRepeatingSync(Runnable runnable, long ticks) {
         return new FutureTaskId(LazyQoL.getEventLoop().scheduleAtFixedRate(() -> runSync(runnable),
